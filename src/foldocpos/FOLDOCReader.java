 ...
 * so that they can be detected by a POSTokenizer (ie braces, brackets
 * etc).<br>
 ...
 * is steady and the NLP parser doesn't get overloaded. <br/>
 *
 * A known bug: words connected by one of the three ('/', ':', '|') 
 * puntuation marks will not be processed properly. E.g. this/is/wow 
 * will become thisiswow. Possible remedy: upgrade GROK?
 */
public class FOLDOCReader extends LineNumberReader {
		...
		public int read() throws IOException {
			...
			for (int c = readOne();; c = readOne()) { //continue until a return
				if (c < 0 || Character.isLetterOrDigit((char)c) || c == '&' || c == '#' || c == ';')
					return egCheck(c);
				...
				/*Everything NOT in this switch will be ignored from the input*/
				case('-'):
...
*/
		...
		public String nextDef() throws IOException {
			...
			if (defWaiting) {
				...
				//System.err.println("Line# "+(getLineNumber()+1)+": "+l);
