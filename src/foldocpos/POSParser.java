...
public class POSParser extends Parser {
	 ...
	 */
	protected boolean firstSignificant = false;
	protected boolean firstSentence = false;
	/** DEBUGGING for classifying tags */
	protected HashMap tagSet;
	/** DEBUGGING for classifying keywords */
	protected HashMap sentence = new HashMap();

	//double prog; //progress modifier: increase * PROGRESS_MOD each iter
	protected boolean syn; 	//indicates synonym link
	protected Node cur; 		//current node
	protected Token keyword;
	
	protected POSTokenizer pt;
	...
	/**
	 * Add the "word" to the wordlist of that "tag"
	 */
	...
	protected void dumpNode(String key) {
	...
	public void token(Token tok, String tag) {
		...
		if (tok.getFlags() == Token.EOP){
			...
			// if reference tag (e.g. a URL or a URI or simply a number ID)
			try {
				Integer.parseInt(tok.toString()); //this only allow an integer!
