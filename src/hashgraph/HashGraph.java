...
public class HashGraph implements Cloneable, java.io.Serializable {
	...
	public boolean FLAG_BACK_LINK = true;
	...
	/** The names of the source dictionary/glossary files */
	public Vector<String> sourceFilenames;
	 ...
	 */
	public Iterator<Node> nodeIterator() { /*fold00*/
		return (Iterator<Node>)nodes.values().iterator();
	}
...
//		boolean wasLabelled; //whether dest was not initally unlabelled
		...
		for (Iterator it = nodeIterator(); it.hasNext();) {
			...
			if ((old = (Node)from.nodes.get(parent.getKey())) != null) {
				for (Iterator it2 = old.values().iterator(); it2.hasNext();) {
					...
					if (kid != null)
						parent.put(kid.getKey(), 
								new Link(parent, kid, l.weight, l.tok, l.inDefinition, l.filename));
				}
				...
				for (Iterator lit = old.backLinks.values().iterator(); lit.hasNext();) {
					...
					if (newPar != null)
						parent.backLinks.put(newPar.getKey(), 
								new Link(newPar, parent, l.weight, l.tok, l.inDefinition, l.filename));
				}
			}
		}
		++modCount;
		...
		for (Iterator it = nodeIterator(); it.hasNext();) {
			...
			for (int back = 0; back < 2; ++back) {
				for (; lit.hasNext();) {
					...
					out.writeUTF(l.tok.toString());
					//out.writeLong(l.lineNo);
					out.writeUTF(l.inDefinition);
					out.writeUTF(l.filename);
					linkCount++;
				}
			...
			for (int i = 0; i < graphSize; ++i) {
				...
				for (int back = 0; back < 2; ++back) {
					for (int j = 0; j < linksSize; ++j) {
						...
						Token tk = new Token(progReadInt(in), in.readUTF()); 
						Link l = new Link(par, kid, wt, tk, in.readUTF(), in.readUTF());
						//System.err.println(l.toStringReasoning()); //debug
						if (back == 0)
	...
	public static int progReadInt(java.io.ObjectInputStream in) /*fold00*/
		...
		for ( int i = (PROG_SIZE_MASK & cur) >> PROG_MASK_SHIFT; i < 0x03; ++i) {
			res <<= 8;
